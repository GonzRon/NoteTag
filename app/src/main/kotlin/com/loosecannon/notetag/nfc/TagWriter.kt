// Interim copy of ServiceTag's app/src/main/kotlin/com/loosecannon/servicetag/nfc/TagWriter.kt (dc1bb1c lineage); Phase G replaces it with nfc-tag-core.
...
object TagWriter {
     ...
     */
    fun inspect(tag: Tag, codec: NoteTagCodec): TagInspection? {
        ...
        Ndef.get(tag)?.let { ndef ->
            return try {
                ...
                val records = try {
                    ...
                    return TagInspection(uid, NoteTagContent.Malformed("NDEF on tag could not be parsed"), emptyList(), ndef.maxSize, ndef.isWritable, false, ndef.canMakeReadOnly())
                }
        ...
        return TagInspection(uid, NoteTagContent.Empty, emptyList(), maxSize = -1, writable = true, needsFormat = true, canLock = true)
    }
